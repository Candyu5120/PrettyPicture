...
if (version_compare(phpversion(), PHP_EDITION, '<')) {
...
switch ($step) {
    ...
    case '4':
        ...
        if (empty($adminEmail)) {
            echo '<script>alert("请输入管理员邮箱!");history.go(-1)</script>';
            exit;
        }
        ...
        if (strlen($adminPassword) < 6) {
            echo '<script>alert("密码长度不能少于6位!");history.go(-1)</script>';
            exit;
        }
        ...
        if (mysqli_connect_errno()) {
            echo '<script>alert("连接数据库失败: " + mysqli_connect_error());history.go(-1)</script>';
            exit();
        }
        ...
        if ($e == 0) {
            ...
            $secretKey = md5($adminEmail . time() . random_int(1000, 9999));
            $currentTime = time();
            $dbPrefix = $dbPrefix ?: 'tp_';
            
            $adminSql = "INSERT INTO `{$dbPrefix}user` (`id`, `role_id`, `username`, `phone`, `email`, `password`, `avatar`, `capacity`, `state`, `Secret_key`, `reg_ip`, `delete_time`, `update_time`, `create_time`) VALUES 
                (1, 1, '管理员', '', '" . mysqli_real_escape_string($conn, $adminEmail) . "', '" . mysqli_real_escape_string($conn, $hashedPassword) . "', 'https://oss.aliyuncs.com/aliyun_id_photo_bucket/default_trade.jpg', '1073741824.00', 1, '" . $secretKey . "', '', NULL, " . $currentTime . ", " . $currentTime . ")";
            
            if (mysqli_query($conn, $adminSql)) {
                $flag = true;
                $msg = ('<div style="font-size:20px">安装成功！SQL成功' . ($t + 1) . '句</div>');
            } else {
                $flag = false;
                $msg = ('<div style="font-size:20px;color:red">创建管理员账户失败: ' . mysqli_error($conn) . '</div>');
            }
        } else {
            $flag = false;
...
function sql_split($sql, $tablepre = 'tp_')
{
    $tablepre = $tablepre ?: 'tp_';
    if ($tablepre != "tp_")
